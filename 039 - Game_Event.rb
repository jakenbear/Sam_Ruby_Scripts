...
class Game_Event < Game_Character
  ...
  def update_self_movement
  #JAR CASE - DO NOT RUN EVENTS OFF SCREEN!!! SWITCH 1613 = ON
    if ($game_switches[1613])
      if near_the_screen? && @stop_count > stop_count_threshold #JAR EDIT OFFSCREEN
        case @move_type
        when 1;  move_type_random
        when 2;  move_type_toward_player
        when 3;  move_type_custom
  ...
  #JAR DEFAULT CASE - RUN EVENTS OFF SCREEN!!! SWITCH 1613 = OFF
    if (!$game_switches[1613])
  ...
  #  ax >= -dx && ax <= dx && ay >= -dy && ay <= dy
   ...
   #JAR CASE - REPLACE NEAR SCREEN DEFINITION - FOR LAG SWITCH 1613 = ON
  def near_the_screen?(dx = 35, dy = 18)
    ax = $game_map.adjust_x(@real_x) - 20
    ay = $game_map.adjust_y(@real_y) - 11.25
    ax >= -dx && ax <= dx && ay >= -dy && ay <= dy
  end
