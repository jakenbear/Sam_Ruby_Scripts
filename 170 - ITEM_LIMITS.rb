...
module YEA
  module LIMIT
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    GOLD_MAX  = 9999999  # Maximum gold.
    GOLD_ICON = 0              # Icon used for gold. Use 0 for text currency.
    GOLD_FONT = 24               # Font size used to display gold.
    TOO_MUCH_GOLD = "A lotta dough!"   # Text used when gold cannot fit.
    ...
    MAXHP_MAX = 9999 # Sets MaxHP to something higher than 9999.
    MAXMP_MAX = 9999 # Sets MaxMP to something higher than 9999.
    PARAM_MAX = 999   # Sets stat max for something higher than 999.
