...
class Window_FileStatus < Window_Base
  ...
  def draw_save_contents
    ...
    if @header[:variables][862] > 0 # non ASM player only
  ...
  def draw_save_portrait
    ...
    if Dir.glob("graphics/Pictures/" + poop.to_s + ".png").length > 0
      if @header[:variables][1010] == 0
        yfix = 0
        xfix = 0
        if @header[:variables][862] == 0
          yfix = 200
          xfix = 240
        end
        draw_image(xfix + xb + YEA::SAVE::SPACING / 2,yb + yfix,"graphics/Pictures/" + poop.to_s + ".png")
      else
  ...
  def get_save_picture_name
    return if @header[:variables].nil?
     if @header[:variables][940] == 0
       ...
       if @header[:variables][862] == 0  # ASM PLAYER
         if @header[:variables][746] > 0 
         poop = "asm_valid"
         else
         poop = "asm_valid_no"
         end
