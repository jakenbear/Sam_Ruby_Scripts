#==============================================================================
# ** Always Sometimes Monsters: Graphics
#------------------------------------------------------------------------------
#  This is the graphics module for ASM and contains initialisation
#  and core functions needed for the game graphics and resolution functions.
#
#  Fullscreen based on Fullscreen++ v2.2 by Zeus81:
#  http://pastebin.com/kc1hzBek | http://www.hbgames.org/forums/viewtopic.php?f=11&t=76084
#==============================================================================
...
class << Graphics
  ...
  unless method_defined?("asm_fullscreen_update")
    HWND     = FindWindow.call('RGSS Player', 0)
    BackHWND = CreateWindowEx.call(0x08000008, 'Static', '', 0x80000000, 0, 0, 0, 0, 0, 0, 0, 0)
    alias asm_fullscreen_resize_screen resize_screen
    alias asm_fullscreen_update        update
  end
...
public
  ...
  def ratio=(r)
    ...
    if r == 0
      ...
      return self.ratio = 0 if w > w_max or h > h_max # change 1 to 0 to re-enable fit to screen
    end
    resize_window(w, h)
  ...
  def resize_screen(width, height)
    asm_fullscreen_resize_screen(width, height)
    self.ratio += 0 # refresh window size
  end
